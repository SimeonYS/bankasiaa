...
class BbankasiaaPipeline:

    # Database setup
    conn = sqlite3.connect('bankasiaa.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bankasiaa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bankasiaa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bankasiaa (date, title, link, content)"
